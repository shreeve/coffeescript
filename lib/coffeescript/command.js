...
// Merge source properties into loop
  ...
  compileCS3 = function(code, options = {}) {
    ...
    ast = parserCS3.parse();
    if (ast != null ? ast.compile : void 0) {
      return ast.compile(parserCS3.parser.yy.backend.compileOptions);
    } else {
      return (new ES5Backend(options)).generate(ast);
    }
  };
    ...
    try {
      ...
      if (opts['cs3-tokens']) {
        ...
        if (opts.print) {
          ...
          return writeJs(base, task.file, task.output, saveTo, null);
        }
      } else if (opts.tokens) {
