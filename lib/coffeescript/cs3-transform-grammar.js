...
(function() {
  ...
  // ==============================================================================
  var action, currentRule, err, fs, grammarContent, grammarPath, i, inGrammar, indent, j, k, len, len1, line, lines, match, outputContent, outputLines, parseArguments, parseConstructorArgs, path, pattern, ref, ref1, stats, successRate, syntaxPath, transformAction, transformExpression, transformed;
  ...
  transformAction = function(actionStr, ruleName) {
    var argList, args, className, cleaned, elements, elseVal, error, match, method, ops, original, prop, props, statements, target, targetRef, test, thenVal, typeValue, useAtSymbol, value;
    if (!actionStr) {
      return null;
    }
    stats.totalRules++;
    ...
    try {
      ...
      if (match = cleaned.match(/^\[(.*)\]$/)) {
        stats.arrayCreation++;
        elements = match[1].split(/,\s*/).map(function(el) {
          if (el.match(/^\$\d+$/)) {
            return `$ref: ${el.slice(1)}`;
          } else {
            return transformExpression(el);
          }
        });
      ...
      if (match = cleaned.match(/^new\s+(\w+)\s*(.*)$/)) {
        ...
        typeValue = useAtSymbol ? "'@'" : `'${className}'`;
      ...
      if (cleaned.indexOf(';') > -1) {
        ...
        ops = statements.map(function(stmt) {
          return transformExpression(stmt.trim());
        });
        return `$seq: [${ops.join(', ')}]`;
      }
      ...
      return `# TODO: Transform manually: ${original}`;
    } catch (error1) {
      error = error1;
      stats.errors.push({
        rule: ruleName,
      ...
      return `# ERROR: ${error.message} - Original: ${original}`;
    }
  };

  // Parse constructor arguments for AST nodes
  parseConstructorArgs = function(argsStr, className) {
    var match, parts, props;
    ...
    if (parts.length === 1) {
      return `, value: ${transformExpression(parts[0])}`;
    } else {
      props = parts.map(function(p, i) {
        return `arg${i + 1}: ${transformExpression(p)}`;
      }).join(', ');
      return `, ${props}`;
    }
  };
  ...
  transformExpression = function(expr) {
    ...
    if (expr === 'true' || expr === 'false' || expr === 'null' || expr === 'undefined' || expr === 'yes' || expr === 'no' || expr === 'on' || expr === 'off') {
      return expr;
    }
    ...
    if (match = expr.match(/^\[(.*)\]$/)) {
      elements = match[1].split(/,\s*/).map(function(el) {
        return transformExpression(el.trim());
      });
      return `{$ary: [${elements.join(', ')}]}`;
    }
  ...
  parseArguments = function(argsStr) {
    ...
    parts = argsStr.split(/,\s*/);
    return parts.map(function(arg) {
      return transformExpression(arg.trim());
    }).join(', ');
  };
...
// Process each line
  for (i = j = 0, len = lines.length; j < len; i = ++j) {
    ...
    if (line.match(/^o\s*=\s*\(/) && !inGrammar) {
      // Add our new o function for pure data
      outputLines.push(`# CS3: The 'o' function now handles pure data actions
o = (patternString, action, options) ->
  patternString = patternString.replace /\\s{2,}/g, ' '
  # Action is now a data object, not a function
  [patternString, action, options]`);
      // Skip the rest of the old o function
      while (i < lines.length - 1 && !lines[i + 1].match(/^[a-z]/i)) {
        i++;
      }
      continue;
    }
    ...
    if (inGrammar) {
      ...
      if (line.match(/^operators\s*=/) || line.match(/^\s+\[/)) {
        outputLines.push(line);
      } else if (!line.match(/^(unwrap|getAddDataToNodeFunctionString|performActionFunctionString)/)) {
        outputLines.push(line);
      }
    }
  }
  ...
  outputContent = outputContent.replace(/module\.exports\s*=[\s\S]*$/, `# CS3: Export with 'grammar' key to signal data-oriented format
module.exports =
  ...
  if (stats.errors.length > 0) {
    ...
    ref = stats.errors.slice(0, 11);
    for (k = 0, len1 = ref.length; k < len1; k++) {
      err = ref[k];
      console.log(`  Rule: ${err.rule}`);
      console.log(`    Action: ${(ref1 = err.action) != null ? ref1.slice(0, 60) : void 0}...`);
