...
(function() {
  ...
  exports.compile = compile = withPrettyErrors(function(code, options = {}) {
    var ast, currentColumn, currentLine, encoded, error, filename, fragment, fragments, generateSourceMap, header, i, j, js, len, len1, map, newLines, nodes, range, ref, sourceCodeLastLine, sourceCodeNumberOfLines, sourceMapDataURI, sourceURL, token, tokens, transpiler, transpilerOptions, transpilerOutput, v3SourceMap;
    // Clone `options`, to avoid mutating the `options` object passed in.
    options = Object.assign({}, options);
    // Check if CS3 mode is requested (but not for AST generation)
    if ((options.cs3 || process.env.COFFEESCRIPT_CS3) && !options.ast) {
      if (process.env.DEBUG_CS3) {
        console.log('[CS3] Using CS3/ES5 backend for compilation');
      }
      try {
    ...
    if (process.env.DEBUG_CS3) {
      console.log('[CS3] Using standard CoffeeScript compiler');
    }
