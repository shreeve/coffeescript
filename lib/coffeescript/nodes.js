  ...
  exports.Block = Block = (function() {
    class Block extends Base {
      ...
      compileWithDeclarations(o) {
        var assigns, declaredVariable, declaredVariables, declaredVariablesIndex, declars, exp, fragments, i, j, k, l, len1, len2, len3, post, ref1, rest, scope, spaced, varIndex, varName;
        fragments = [];
        post = [];
        ref1 = this.expressions;
          ...
          if (declars || assigns) {
            ...
            if (o.es6) {
              ...
              if (declaredVariables.length > 0) {
                ...
                for (varIndex = k = 0, len2 = declaredVariables.length; k < len2; varIndex = ++k) {
                  varName = declaredVariables[varIndex];
                  fragments.push(this.makeCode(varName));
                  if (Object.prototype.hasOwnProperty.call(o.scope.comments, varName)) {
                    fragments.push(...o.scope.comments[varName]);
                  }
                  if (varIndex !== declaredVariables.length - 1) {
                    fragments.push(this.makeCode(', '));
                  }
                }
                if (!assigns) {
              ...
              if (declars) {
                declaredVariables = scope.declaredVariables();
                for (declaredVariablesIndex = l = 0, len3 = declaredVariables.length; l < len3; declaredVariablesIndex = ++l) {
...
// multiline need to be indented.
  ...
  exports.Range = Range = (function() {
    class Range extends Base {
      ...
      compileNode(o) {
        var cond, condPart, from, gt, idx, idxName, known, lowerBound, lt, namedIndex, ref1, ref2, stepCond, stepNotZero, stepPart, to, upperBound, varKeyword, varPart;
        if (!this.fromVar) {
        ...
        varKeyword = o.es6 ? 'let' : 'var';
        varPart = known && !namedIndex ? `${varKeyword} ${idx} = ${this.fromC}` : `${idx} = ${this.fromC}`;
  ...
  exports.ExportDeclaration = ExportDeclaration = class ExportDeclaration extends ModuleDeclaration {
    compileNode(o) {
      ...
      if (!(this instanceof ExportDefaultDeclaration) && (this.clause instanceof Assign || this.clause instanceof Class)) {
        varKeyword = o.es6 ? 'let' : 'var';
        code.push(this.makeCode(`${varKeyword} `));
    ...
    class Assign extends Base {
      ...
      addScopeVariables(o, {allowAssignmentToExpansion = false, allowAssignmentToNontrailingSplat = false, allowAssignmentToEmptyArray = false, allowAssignmentToComplexSplat = false} = {}) {
        ...
        return varBase.eachName((name) => {
          var alreadyDeclared, commentFragments, commentsNode, message, ref1;
          ...
          if (this.moduleDeclaration) {
            ...
            if (this.canBeConst && name.isDeclaration) {
              if ((ref1 = o.scope.getVariable(name.value)) != null) {
                ref1.alreadyDeclared = true;
              }
            }
      ...
      compileNode(o) {
        var answer, compiledName, isValue, name, properties, prototype, ref1, ref2, ref3, ref4, ref5, val, varName, variable;
        ...
        if (o.es6 && !this.context && ((ref5 = this.variable.base) != null ? ref5.isDeclaration : void 0) && this.variable.base instanceof IdentifierLiteral && o.level <= LEVEL_LIST) {
          // Check if we're assigning a function or class
          isFunction = this.value instanceof Code || this.value instanceof Class;
          if (isFunction && !o.scope.parent) {
            // Use const for top-level function/class declarations
            answer = [this.makeCode('const ')].concat(compiledName, this.makeCode(' = '), val);
            // Mark variable as declared inline to avoid hoisting
            varName = this.variable.base.value;
            variable = o.scope.getVariable(varName);
            if (variable) {
              variable.declaredInline = true;
            }
          } else {
      ...
      compileNode(o) {
        ...
        if (this.range) {
          ...
          if (!this.object && !this.from) {
            ...
            if (o.es6 && !this.step && !this.index && !this.pattern) {
              forPartFragments = [this.makeCode(`${name} of ${svar}`)];
        ...
        if (this.object) {
          if (o.es6 && !this.own) {
            ...
            forPartFragments = [this.makeCode(`${kvar} of Object.keys(${svar})`)];
          } else {
