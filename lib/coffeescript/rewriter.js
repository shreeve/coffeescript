...
(function() {
  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      looksAppendable(j) {
        if (this.looksObjectish(j) || this.indexOfTag(j, IMPLICIT_CALL) > -1) {
          return !this.findTagsBackwards(j - 2, ['CLASS', 'EXTENDS', 'IF', ':', 'FINALLY', 'TRY', 'CATCH', 'SWITCH', 'LEADING_WHEN', 'FOR', 'WHILE', 'UNTIL']);
        }
        return false;
      }
      ...
      adjustCoffeeTags() {
        return this.scanTokens(function(token, i, tokens) {
          var c, f, h, m, o, s, t;
          if (token[0] === 'CT') {
            m = tokens[i + 1][0] !== 'TERMINATOR';
            t = token[1];
            if (t.slice(-2) === '()') {
              this.coffeeTags = t.slice(0, -2);
              tokens.splice(i + 0, m ? 1 : 2);
              return 0;
            }
            f = generate('IDENTIFIER', 'coffeeTags');
            f.spaced = true;
            h = generate('IDENTIFIER', this.coffeeTags || (this.coffeeTags = 'h'));
            c = generate(',', ',');
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (tag === 'INDENT' && inImplicit()) {
            ...
            if (prevTag !== '=>' && prevTag !== '->' && prevTag !== '[' && prevTag !== '(' && prevTag !== ',' && prevTag !== '{' && prevTag !== 'ELSE' && prevTag !== '=') {
              if (this.looksAppendable(i + 1)) { // look at implicitObjectContinues() also
