...
(function() {
  ...
  ES5Backend = class ES5Backend {
    ...
    evaluateDirective(directive, frame, ruleName = null) {
      var args, argsNode, array, attempt, attemptNode, awaitTag, awaitVal, body, bodyNode, bound, clause, condition, context, delimiter, elseBody, elseNode, ensure, exclusive, expression, expressions, flags, flip, forNode, from, fullRegex, generated, guard, i, idx, index, inner, invertOperator, j, key, left, len, len1, name, nameNode, nodeType, object, objects, op, originalOperator, own, ownTag, params, paramsNode, parent, parsedValue, pattern, prop, properties, quote, recovery, ref, ref1, ref10, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, result, right, source, sourceObj, splat, step, tail, templateArg, test, to, vNode, value, valueNode, variable, variableNode;
        ...
        // $ast directive (AST node creation)
          ...
          switch (nodeType) {
            ...
            case 'While':
              ...
              body = this.evaluateDirective(directive.body, frame, ruleName);
              bodyNode = Array.isArray(body) ? new nodes.Block(this.filterNodes(body)) : body;
              return new nodes.While(condition, null, bodyNode);
            case 'For':
              // For nodes are built incrementally via $ops: 'loop', addSource
              // The initial For just has body and name/index
              body = this.evaluateDirective(directive.body, frame, ruleName);
              ...
              ownTag = this.evaluateDirective(directive.ownTag, frame, ruleName);
              object = this.evaluateDirective(directive.object, frame, ruleName);
              from = this.evaluateDirective(directive.from, frame, ruleName);
              awaitVal = this.evaluateDirective(directive.await, frame, ruleName);
              awaitTag = this.evaluateDirective(directive.awaitTag, frame, ruleName);
              
              // Handle body - often starts as empty array
              bodyNode = (body != null ? body.expressions : void 0) ? (expressions = this.filterNodes(Array.isArray(body.expressions[0]) ? body.expressions[0] : body.expressions), new nodes.Block(expressions)) : Array.isArray(body) ? new nodes.Block(this.filterNodes(body)) : body ? new nodes.Block([body]) : new nodes.Block([]);
              
              // Unwrap arrays for name and index (they come as arrays from the parser)
              ...
              if (awaitVal) {
                sourceObj.await = awaitVal;
              }
              if (awaitTag) {
                sourceObj.awaitTag = awaitTag;
              }
              
              // Create the For node with body and source object
              // The constructor expects (body, source) where source is the object with all properties
    ...
    solarNodeToClass(solarNode) {
      var array, body, clause, first, flags, from, fullRegex, i, inner, len, object, objects, pattern, prop, properties, second, source, to, value, valueNode, variable;
      ...
      switch (solarNode.type) {
        ...
        case 'UndefinedLiteral':
          return new nodes.UndefinedLiteral();
        case 'Value':
          if (solarNode.val || solarNode.value) {
            inner = this.solarNodeToClass(solarNode.val || solarNode.value);
          }
          ...
          if (inner) {
            ...
            return new nodes.Value(new nodes.Literal("undefined"));
          }
          break;
        case 'Op':
          if (solarNode.left) {
    ...
    applyOperation(directive, frame, ruleName) {
      var bodyNode, evaluated, i, item, j, len, len1, loopNode, propNode, propRaw, ref1, ref2, ref3, ref4, ref5, ref6, ref7, result, sourceInfo, target, targetNode, targetRaw, value;
      switch (directive.$ops) {
        ...
        case 'loop':
          ...
          if (directive.addSource != null) {
            ...
            console.error("[loop.addBody] directive:", JSON.stringify(directive.addBody));
            loopNode = this.evaluateDirective(directive.addBody[0], frame, ruleName);
            bodyNode = this.evaluateDirective(directive.addBody[1], frame, ruleName);
            console.error("[loop.addBody] evaluated body:", bodyNode);
            ...
            if (bodyNode != null ? bodyNode.type : void 0) {
              ...
              // If it's a string placeholder like "Body $2", it means evaluation failed
              console.error("[loop.addBody] Got string body - likely missing implementation");
              return loopNode;
            }
            
            // Ensure body has locationData if it's a node
