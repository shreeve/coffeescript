        ...
        // $ast directive (AST node creation)
          ...
          switch (nodeType) {
            ...
            case 'Call':
              ...
              soak = this.evaluateDirective(directive.soak, frame, ruleName);
              if (argsNode.length === 1) {
                argNode = argsNode[0];
                // Check if it's a string or string-like node
                if (argNode instanceof nodes.StringLiteral || argNode instanceof nodes.StringWithInterpolations) {
                  // This is a tagged template, not a regular call
                  templateArg = argNode instanceof nodes.StringLiteral ? nodes.StringWithInterpolations.fromStringLiteral(argNode) : argNode;
                  // Tagged templates should never have soak (no typeof check)
                  return new nodes.TaggedTemplateCall((variableNode instanceof nodes.Value ? variableNode : new nodes.Value(variableNode)), templateArg, false);
                }
              }
              return new nodes.Call((variableNode instanceof nodes.Value ? variableNode : new nodes.Value(variableNode)), argsNode, soak, this.evaluateDirective(directive.token, frame, ruleName));
            case 'TaggedTemplateCall':
              ...
              return new nodes.TaggedTemplateCall((vNode instanceof nodes.Value ? vNode : new nodes.Value(vNode)), templateArg, this.evaluateDirective(directive.soak, frame, ruleName));
            case 'Assign':
        ...
        case 'loop':
          ...
          if (directive.addSource != null) {
            ...
            if (bodyNode) {
              ...
              if (!(bodyNode instanceof nodes.Base)) {
