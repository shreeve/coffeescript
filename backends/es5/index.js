...
(function() {
  ...
  ES5Backend = class ES5Backend {
    ...
    evaluateProps(directive, props, frame, ruleName) {
      var j, len, prop, result;
      result = {};
      for (j = 0, len = props.length; j < len; j++) {
        prop = props[j];
        if (directive[prop] != null) {
          result[prop] = this.evaluateDirective(directive[prop], frame, ruleName);
        }
      }
      return result;
    }
    ...
    lazyEval(directive, prop, frame, ruleName) {
      if (directive[prop] != null) {
        return this.evaluateDirective(directive[prop], frame, ruleName);
      } else {
        return null;
      }
    }
    ...
    createIf(directive, frame, ruleName, defaultType = 'if') {
      var body, bodyNode, condition, elseBody, elseNode, ifNode, opts, postfix, props, type;
      // Batch evaluate properties
      props = this.evaluateProps(directive, ['condition', 'body', 'elseBody', 'type', 'postfix'], frame, ruleName);
    ...
    createSwitch(directive, frame, ruleName) {
      var cases, casesNode, otherwise, props, subject;
      props = this.evaluateProps(directive, ['subject', 'cases', 'otherwise'], frame, ruleName);
    ...
    createCatch(directive, frame, ruleName) {
      ...
      body = this.lazyEval(directive, 'recovery', frame, ruleName) || this.lazyEval(directive, 'body', frame, ruleName);
      // CS3 uses 'variable' or 'errorVariable' for the error parameter
      error = this.lazyEval(directive, 'variable', frame, ruleName) || this.lazyEval(directive, 'errorVariable', frame, ruleName);
    ...
    createWhile(directive, frame, ruleName) {
      var body, bodyNode, condition, finalBody, guard, invert, isLoop, opts, props, whileNode;
      // Batch evaluate properties
      props = this.evaluateProps(directive, ['condition', 'body', 'guard', 'isLoop', 'invert'], frame, ruleName);
    ...
    createTry(directive, frame, ruleName) {
      var attempt, attemptNode, catchDirective, ensure, ensureNode, props, recovery;
      props = this.evaluateProps(directive, ['attempt', 'catch', 'ensure'], frame, ruleName);
    ...
    createStringWithInterpolations(directive, frame, ruleName) {
      var body, bodyNode, bodyNodes, props, quote;
      props = this.evaluateProps(directive, ['body', 'quote'], frame, ruleName);
      ({body, quote} = props);
    ...
    createParam(directive, frame, ruleName) {
      var atValue, i, j, len, name, obj, prop, props, ref1, ref2, splat, value;
      props = this.evaluateProps(directive, ['name', 'value', 'splat'], frame, ruleName);
    ...
    createClass(directive, frame, ruleName) {
      var body, bodyNode, parent, props, variable;
      props = this.evaluateProps(directive, ['variable', 'parent', 'body'], frame, ruleName);
    ...
    createRegexLiteral(directive, frame, ruleName) {
      var delimiter, flags, fullRegex, pattern, props, value;
      props = this.evaluateProps(directive, ['value', 'delimiter', 'pattern', 'flags'], frame, ruleName);
    ...
    createRange(directive, frame, ruleName) {
      var exclusive, from, fromNode, props, tag, to, toNode;
      props = this.evaluateProps(directive, ['from', 'to', 'exclusive', 'equals'], frame, ruleName);
      ({from, to} = props);
      // Evaluate the exclusive flag - it can be a directive or a boolean
      exclusive = typeof props.exclusive === 'boolean' ? props.exclusive : props.equals != null ? props.equals === 'exclusive' : false;
    ...
    createFor(directive, frame, ruleName) {
      var body, bodyNode, forNode, from, guard, incrementVar, index, isAwait, loopVar, name, nameArray, object, originalCompileNode, own, props, source, sourceObj, step;
      // Batch evaluate only the properties that exist
      props = this.evaluateProps(directive, ['body', 'source', 'guard', 'name', 'index', 'step', 'own', 'object', 'from', 'await'], frame, ruleName);
    ...
    createAssign(directive, frame, ruleName) {
      var context, options, props, value, variable;
      ...
      if (directive.context === 'object' && (directive.expression != null)) {
        ...
        props = this.evaluateProps(directive, ['value', 'expression'], frame, ruleName);
        ...
        // Regular assignment
        props = this.evaluateProps(directive, ['variable', 'value', 'operator', 'context', 'originalContext'], frame, ruleName);
        variable = props.variable;
        value = props.value;
        // For compound assignments, use the operator as the context
        context = props.operator || props.context;
        options = {};
        if (props.originalContext != null) {
          options.originalContext = props.originalContext;
        }
