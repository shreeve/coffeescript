...
# Try to detect which CoffeeScript to use
CoffeeScript = do ->
  # Check if we're running with global CoffeeScript 2.7.0+
  try
    cs = require 'coffeescript'
    [major, minor] = cs.VERSION.split('.').map(Number)
    if major > 2 or (major == 2 and minor >= 7)
      console.log "# Using global CoffeeScript #{cs.VERSION}"
      return cs
  catch
    # Global not available or wrong version

  # Try local v28 CoffeeScript
  try
    console.log "# Using local v28 CoffeeScript"
    require '../lib/coffeescript'
  catch
    console.log "# Warning: Could not load CoffeeScript module"
    process.exit 1
