...
export compile = withPrettyErrors (code, options = {}) ->
  ...
  # FIXME: This is most certainly going to break SourceMaps, so we need to fix it.
  # Adjust the generated code header and top-level var declarations
  header = if options.header then "// Generated by CoffeeScript #{VERSION}\n" else ""
  if match = js.match(/^((?:(?:\/\/.*|\s*)(?:\n|$))*?)(var\s+[\s\S]*?;\n+)/m)
