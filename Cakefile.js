...
cake doc:source`, {
  ...
  runTests = function(CoffeeScript) {
    var asyncTests, code, currentFile, error, file, filename, files, func, i, len, literate, name, onFail, ref, skipUnless, startTime, testFilesToSkip;
    ...
    files = fs.readdirSync('test').filter(function(filename) {
      return indexOf.call(testFilesToSkip, filename) < 0;
    });
    startTime = Date.now();
    for (i = 0, len = files.length; i < len; i++) {
      file = files[i];
